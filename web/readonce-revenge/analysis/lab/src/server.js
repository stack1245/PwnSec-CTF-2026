...
app.get("/review", (req, res) => {
  if (!req.session.admin || !reviewMatches(req)) {
...
app.get("/sandbox", (req, res) => {
  if (!req.session.admin || !reviewMatches(req) || !currentReview.document) {
    res.status(404).type("text/plain").send("not found");
    return;
  }
...
app.post("/complete", (req, res) => {
  ...
  if (currentReview && currentReview.id === id && currentReview.prepared && req.session.admin && state === currentReview.nonce) {
    ...
    console.log("[complete] approved", id);
  }
...
app.get("/reports/check", (req, res) => {
  console.log("[check]", req.url, req.get("sec-fetch-site"), req.get("sec-fetch-dest"), currentReview && { prepared: currentReview.prepared, approved: currentReview.approved, used: currentReview.used, visited: currentReview.visited });
