...
function watchDocument(page, report, entryUrl) {
  ...
  page.on("request", (request) => {
    console.log("[watch]", request.isNavigationRequest(), request.frame() === page.mainFrame(), request.url());
    ...
    if (isReportDocument(next, report)) {
      ...
      console.log("[watch] finalized", report.finalized, {approved: report.approved, diverged});
      return;
    }
...
async function review(report) {
  ...
  try {
    ...
    console.log("[cdp]", browser.wsEndpoint());
